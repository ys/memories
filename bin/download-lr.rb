...
class Downloader
  ...
  def run
    ...
    `mkdir -p data/albums`
    File.open("data/albums/#{@year}.json", "w") { |file| file.write(JSON.dump(JSON.parse(resp))) }
...
albums = YAML.load(File.read("data/albums.yaml"), symbolize_names: true)

albums[:years].each do |year, info|
